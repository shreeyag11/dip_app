<template>
  <b-modal
    :active.sync="$store.state.ass1.isAsciiModalActive"
    has-modal-card
    trap-focus
...
export default {
  ...
  methods: {
    ...
    submitModal() {
      ...
      if (!this.lightCharInvalid || !this.darkCharInvalid || !this.thresholdInvalid) {
      ...
      a.setAttribute('download', this.$store.state.ass1.file.name.replace('.txt', '_ascii_art.txt'));
      a.click();
      a.remove();
    },
  },
